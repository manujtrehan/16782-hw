...
unordered_map<set<GroundedCondition, CustomGC>, shared_ptr<Node>, NodeHasher, NodeComparator> nodes_heuristics;
unordered_set<set<GroundedCondition, CustomGC>, NodeHasher, NodeComparator> closed;
unordered_set<set<GroundedCondition, CustomGC>, NodeHasher, NodeComparator> closed_heuristics;
...
void addHeuristicEffectToState(
        set<GroundedCondition, CustomGC>& state,
        unordered_set<GroundedCondition, GroundedConditionHasher, GroundedConditionComparator>& effects)
{
    for(auto effect : effects)
    {
        if(effect.get_truth()) // empty delete list - don't delete any effect
        {
            state.insert(effect);
        }
    }
...
int computeHeuristic(
        set<GroundedCondition, CustomGC>& goal,
        priority_queue<shared_ptr<Node>, vector<shared_ptr<Node> >, decltype(compare)>& open_queue_heuristics)
{
	int cost = 1;
    int count = 0;
	while(!open_queue_heuristics.empty())
	{
        cout << "Heuristic count: " << count << endl;
        ++count;
        // cout << "Open queue size: " << open_queue_heuristics.size() << endl;
		shared_ptr<Node> s = open_queue_heuristics.top();
		open_queue_heuristics.pop();

		if(closed_heuristics.find(s->conditions) != closed_heuristics.end()) continue; // skip if already visited
		closed_heuristics.insert(s->conditions); // add to closed_heuristics list
        ...
        vector<MultiDimMap*> output;
        findInActionMap(s->conditions, &action_map, nullptr, output, 0); // find possible actions based on current state
        ...
        for(MultiDimMap* solution_ptr : output)
        {
            for(auto effect_action_pair : solution_ptr->value)
            {
                ...
                addHeuristicEffectToState(child_conditions, effect);

                if(closed_heuristics.find(child_conditions) == closed_heuristics.end())
                {
                    if(nodes_heuristics.find(child_conditions) == nodes_heuristics.end())
                    {
                        shared_ptr<Node> newNode = make_shared<Node>(child_conditions);
                        nodes_heuristics[child_conditions] = newNode;
                    }
                    if(nodes_heuristics[child_conditions]->fVal > (s->fVal + cost))
                    {
                        nodes_heuristics[child_conditions]->fVal = s->fVal + cost;
                        nodes_heuristics[child_conditions]->parent = s;
                        nodes_heuristics[child_conditions]->parent_action = grounded_action;
                        open_queue_heuristics.push(nodes_heuristics[child_conditions]);
                    }
                }
            }
        }
        ...
        for(MultiDimMap* solution_ptr : output)
        {
            for(auto effect_action_pair : solution_ptr->value)
            {
                ...
                if(closed.find(child_conditions) == closed.end())
                {
                    ...
                    {
                        priority_queue<shared_ptr<Node>, vector<shared_ptr<Node> >, decltype(compare)> temp_queue(compare);
                        // nodes_heuristics.clear();
                        closed_heuristics.clear();
                        shared_ptr<Node> heur = make_shared<Node>(child_conditions);
                        heur->fVal = 0;
                        nodes_heuristics[heur->conditions] = heur;
                        temp_queue.push(heur);
                        int h = computeHeuristic(goal, temp_queue);
...
list<GroundedAction> planner(Env* env)
{
    ...
    shared_ptr<Node> start_h = make_shared<Node>(init);
    start_h->fVal = 0;
    nodes_heuristics[init] = start_h;
    priority_queue<shared_ptr<Node>, vector<shared_ptr<Node> >, decltype(compare)> temp_queue(compare);
    temp_queue.push(start_h);
    int h_start = computeHeuristic(goal, temp_queue);
