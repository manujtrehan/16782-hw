        ...
        )
{
    ...
    while(!openQueue.empty())
    {
        ...
        // {
        //     ++count;
        //     int cost = 0; // same cost for all the goals to common goal
        ...
        {
            // if(s->time < goals[s->mapIndex])
            // {
            //     int futureCost = (int) map[s->mapIndex] * (goals[s->mapIndex] - s->time);
            //     s->f = s->g + futureCost;
            //     openQueue.insert(std::upper_bound(openQueue.begin(), openQueue.end(), s, nodeCompare), s);
            // }
            // else
            // {
